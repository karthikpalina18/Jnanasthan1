...
// Store for rooms and user sockets
const rooms = {};
const userSockets = {};
...
io.on('connection', (socket) => {
  ...
  socket.on('join-room', ({ roomId, username }) => {
    if (!roomId || !username) {
      return socket.emit('error', { message: 'Room ID and username required' });
    }
  ...
  // Chat messages
  socket.on('send-message', ({ roomId, message }) => {
    if (!roomId || !message?.content || !message?.sender) {
      return socket.emit('error', { message: 'Invalid message data' });
    }
    
    // Limit message length
    if (message.content.length > 1000) {
      message.content = message.content.substring(0, 1000) + '... (truncated)';
    }
  ...
  socket.on('direct-message', async (data) => {
    try {
      ...
      // Note: You'll need to import the Conversation model at the top
...
app.post('/get-token', (req, res) => {
  ...
  if (roomName.length > 50 || participantName.length > 50) {
    return res.status(400).json({ error: 'Names too long' });
  }
