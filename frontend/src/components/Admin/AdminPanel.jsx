  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // You'll need to implement these endpoints on your backend
        const [usersRes, quizzesRes, attemptsRes] = await Promise.all([
          axios.get('https://jnanasthan1-production.up.railway.app/auth/users/count', {
            headers: { 'x-auth-token': token }
          }),
          axios.get('https://jnanasthan1-production.up.railway.app/quiz/count', {
            headers: { 'x-auth-token': token }
          }),
          axios.get('https://jnanasthan1-production.up.railway.app/attempt/count', {
            headers: { 'x-auth-token': token }
          })
        ]);
        ...
        // Optional: Get recent users and quizzes
        const [recentUsersRes, recentQuizzesRes] = await Promise.all([
          axios.get('https://jnanasthan1-production.up.railway.app/auth/users/recent', {
            headers: { 'x-auth-token': token }
          }),
          axios.get('https://jnanasthan1-production.up.railway.app/quiz/recent', {
            headers: { 'x-auth-token': token }
          })
        ]);
