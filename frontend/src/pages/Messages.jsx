...
const Messages = () => {
  ...
  useEffect(() => {
    const fetchConnections = async () => {
      ...
      setLoading(true);
      try {
        const response = await axios.get('https://jnanasthan1-production.up.railway.app/connection/myConnections', config);
        setConnections(response.data);
        setLoading(false);
      } catch (err) {
        setError('Failed to fetch connections. ' + (err.response?.data?.message || err.message));
  ...
  useEffect(() => {
    const fetchMessages = async () => {
      ...
      setLoading(true);
      try {
        const response = await axios.get(`https://jnanasthan1-production.up.railway.app/message/${selectedConnectionEmail}`, config);
        setMessages(response.data);
        setLoading(false);
      } catch (error) {
        console.error('Error fetching messages:', error);
  ...
  const handleSendMessage = async (e) => {
    ...
    try {
      const response = await axios.post('https://jnanasthan1-production.up.railway.app/message', messageData, config);
      setMessages([...messages, response.data]);
      setMessageContent('');
      setLoading(false);
    } catch (error) {
