...
//     try {
//       const res = await axios.post('https://jnanasthan1-production.up.railway.app/auth/login', { 
//         email, 
//         password 
//       });
...
const Login = () => {
  ...
  const checkServerHealth = async () => {
    try {
      const response = await fetch('https://jnanasthan1-production.up.railway.app/health', {
        method: 'GET',
        timeout: 5000
      });
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      const response = await fetch('https://jnanasthan1-production.up.railway.app/auth/login', {
        method: 'POST',
        headers: {
      ...
      // Store auth data in memory (not localStorage due to Claude restrictions)
      ...
      >
          ...
          <div className="mb-6 p-3 bg-yellow-100 border-l-4 border-yellow-500 text-yellow-700 rounded">
            <div className="flex items-center">
              ...
              <div>
                ...
                <p className="text-sm">The server is currently offline. Please try again later or contact support.</p>
              </div>
            </div>
          </div>
        )}
        ...
        <div>
          ...
          <button
            ...
            className={`w-full py-4 rounded-xl font-medium text-white shadow-lg 
              transition-all duration-300 ease-in-out transform hover:scale-105 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-purple-500
              ${isLoading || serverStatus === 'offline' ? 'bg-gray-400 cursor-not-allowed' : 'bg-gradient-to-r from-purple-600 to-indigo-600 hover:from-purple-700 hover:to-indigo-700'}`}
            disabled={isLoading || serverStatus === 'offline'}
          >
            {isLoading ? (
              <span className="flex items-center justify-center">
        ...
        <div className="mt-8 text-center text-gray-500 text-sm">
          ...
          {serverStatus === 'offline' && (
            <p className="mt-2 text-xs">
              Having issues? Contact support at <a href="mailto:support@jnanasthan.com" className="text-purple-600">support@jnanasthan.com</a>
            </p>
          )}
        </div>
      </div>
